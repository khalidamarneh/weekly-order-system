...
// frontend/src/components/admin/Inbound_Invoices/InvoiceFromOrder.jsx
...
const InvoiceFromOrder = ({ isDarkMode = false, onNavigate, onViewInvoiceHistory, setActiveSubTab }) => {
    ...
    const loadOrders = async () => {
        try {
            const res = await api.get('/api/admin/inbound-orders');
    ...
    // Process selected orders for invoice
    const processSelectedOrders = () => {
        ...
        selectedOrdersData.forEach((order, orderIndex) => {
            // Use the first order's customer info
            if (orderIndex === 0 && order.client) {
            ...
            order.items.forEach(item => {
                ...
                allItems.push({
                    id: `order-${order.id}-item-${item.id}-${Date.now()}`,
                    productId: item.product?.id || null,
                    name: item.product?.name || item.description || 'Unlisted Item',
                    partNo: item.product?.partNo || '',
                    description: item.product?.description || '',
                    image: item.product?.image || item.imagePath || '',
                    quantity: item.quantity,
                    unitPrice: salePrice,
                    unitCost: costPrice,
                    totalPrice: itemTotal,
                    sourceOrderId: order.id,
                    sourceOrderNumber: order.orderNumber || `#${order.id}`,
                    markupAmount: costPrice > 0 ? salePrice - costPrice : 0,
                    markupPercentage: costPrice > 0 ? ((salePrice - costPrice) / costPrice) * 100 : 0
                });
            });
        });
        ...
        setInvoice(prev => ({
            ...
            customerEmail: prev.customerEmail || customerEmail,
            customerPhone: prev.customerPhone || customerPhone,
        ...
        setSelectedOrders(new Set());
        alert(`Added ${allItems.length} items from ${selectedOrders.size} orders to invoice`);
    };
    ...
    // Load invoice for edit - FIXED VERSION
    ...
    const generatePrintHtml = () => {
        const itemsHtml = invoice.items.map((it, idx) => `
            <tr style="background:${idx % 2 === 0 ? '#fff' : '#f8faf0'}">
                ...
                <td style="padding:8px">${it.image ? `<img src="${it.image}" style="width:48px;height:48px;object-fit:cover;border-radius:4px;margin-right:8px;vertical-align:middle" />` : ''}${it.name}<br/><small>Part: ${it.partNo || '-'}</small>${it.sourceOrderNumber ? `<br/><small>From: ${it.sourceOrderNumber}</small>` : ''}</td>
...
// REPLACE your current navigation functions with this:
const handleBackToDashboard = () => {
    console.log('🔄 Direct navigation to dashboard');
    // Use the same function that the feature cards use
...
const handleViewInvoiceHistory = () => {
    console.log('🔄 Direct navigation to history');
    // Use the same function that the feature cards use  
    if (setActiveSubTab && typeof setActiveSubTab === 'function') {
...
<div className={`p-4 sm:p-6 rounded-xl ${
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
                <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2 sm:gap-3 truncate">
                    <ClipboardListIcon className="h-6 w-6 sm:h-7 sm:w-7 text-green-500 flex-shrink-0" />
                    <span className="truncate">Create Invoice from Orders</span>
                </h1>
                {/* FIXED: Replace the broken back button */}
                <button
                    onClick={handleBackToDashboard}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors text-sm mt-2 ${
                        isDarkMode 
                ...
                <p className="text-gray-400 mt-1 text-sm sm:text-base truncate">
                    Generate customer invoices from inbound orders
                </p>
            </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3 justify-center sm:justify-start">
                        ...
                        <button
                            ...
                            className={`${buttonBaseClass(isDarkMode, 'green', 'sm')} ${
                                (loading || invoice.status === 'SENT') ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`${buttonBaseClass(isDarkMode, 'purple', 'sm')} ${
                                (loading || invoice.items.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                        >
                            <DocumentTextIcon className="h-4 w-4" />
                        ...
                        <button
                            onClick={sendInvoice}
                            disabled={loading}
                            className={`${buttonBaseClass(isDarkMode, 'cyan', 'sm')} ${
                            ...
                            <div className="overflow-x-auto rounded-lg border">
                                ...
                                {invoice.items.length === 0 && (
                                    <div className={`text-center py-12 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'
                                        ...
                                        <p className="text-sm mt-1">Add items from inbound orders or create manual items</p>
                                    </div>
                                )}
                            </div>

                            {/* Order Source Summary */}
                            {invoice.items.some(item => item.sourceOrderId) && (
                                <div className={`mt-4 p-4 rounded-lg border ${
                                    ...
                                    <div className="text-sm">
                                        Items from {new Set(invoice.items.map(item => item.sourceOrderId).filter(Boolean)).size} orders
                                    </div>
                                </div>
                            )}
            ...
            {/* Order Selector Modal - Optimized for Stability, Size & Scroll */}
{showOrderSelector && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4">
    ...
    >
        ...
        <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs">
                      ...
                      {formatMoney(calculateOrderTotal(order))}
                    </div>
                  </div>
                </div>
                <div className="text-right">
          ...
          {orders.length === 0 && (
            <div className="text-center py-8">
              <ClipboardListIcon className="h-10 w-10 mx-auto mb-3 text-gray-400" />
              <p
      ...
      {/* Footer */}
      <div
        className={`px-4 py-3 border-t flex-shrink-0 flex flex-col sm:flex-row justify-end gap-2 ${
          isDarkMode
            ? 'border-gray-700 bg-gray-900'
            : 'border-gray-200 bg-gray-50'
        }`}
      >
        ...
        >
          <PlusIcon className="h-4 w-4" />
          Add Selected Orders ({selectedOrders.size})
        </button>
      </div>
    </div>
  </div>
            ...
            {showCustomerModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4">
                    <div className={`w-full max-w-4xl rounded-xl shadow-2xl max-h-[95vh] overflow-hidden flex flex-col ${
...
export default InvoiceFromOrder;